...
func (res *Resolver) Run(ctx context.Context) <-chan error {
	dns.HandleFunc(res.config.Domain+".", panicRecover(res.HandleSwan))
	dns.HandleFunc(".", panicRecover(res.HandleNonSwan(res.defaultFwd)))
...
func (res *Resolver) HandleNonSwan(fwd Forwarder) func(
